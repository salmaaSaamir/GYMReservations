...
export class ReservationCalenderComponent {
  ...
  closeReservation() {
    ...
    this.Reservation = { Id: 0, ClassId: 0, MemberId: 0 }
  ...
  async SaveReservation() {
    ...
    try {
      ...
      console.error(error);
      this.toaster.error("Failed to save reservation");
    } finally {
      this.IsSpinner = false;
    }
  }
  ...
  async GetMemberId() {
    ...
    try {
      const res: any = await lastValueFrom(
        this.CalenderService.GetMmeberById(this.Reservation.MemberId.toString())
      );

      if (res.State && res.Data[0] != null) {
        this.Reservation.MemberId = +res.Data[0].Id
        this.SaveReservation()
      }else{
        this.toaster.error("Member Id Not Correct");
      }
    } catch (error: any) {
      console.error('Error Get Mmember ', error);
      this.toaster.error(error.error?.ErrorMessage || "Error Get Member Id");
    }
  }
